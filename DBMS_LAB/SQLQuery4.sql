...
SELECT A.aname
FROM AUTHOR A
JOIN CATALOGUE C ON A.authorid = C.authorid
JOIN ORDER_DET O ON C.bookid = O.bookid
GROUP BY A.aname, C.bookid
...
SELECT COUNT(DISTINCT ordno) AS NO_OF_ORDERS, bookid
FROM ORDER_DET
GROUP BY bookid
HAVING SUM(qty) <= ALL (
    SELECT SUM(qty)
